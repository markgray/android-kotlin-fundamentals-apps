...
class MainActivity : AppCompatActivity() {
     ...
     * `R.id.root_view` and call the [ViewCompat.setOnApplyWindowInsetsListener] method to
     * set an [OnApplyWindowInsetsListener] to take over over the policy for applying window
     ...
     * right margin set to `systemBars.right`, the top margin set to `systemBars.top`, and
     * the bottom margin set to the maximum of the system bars bottom inset and the IME bottom
     * inset. Finally it returns [WindowInsetsCompat.CONSUMED] to the caller (so that the window
    ...
    // TASK (01) Create the new TitleFragment
    // Select File->New->Fragment->Fragment (Blank)

    // TASK (02) Clean up the new TitleFragment
    // In our new TitleFragment

    // TASK (03) Use DataBindingUtil.inflate to inflate and return the titleFragment in onCreateView
    // In our new TitleFragment
