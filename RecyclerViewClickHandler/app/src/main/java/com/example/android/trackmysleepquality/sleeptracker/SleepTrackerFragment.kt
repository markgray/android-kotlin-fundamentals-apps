...
class SleepTrackerFragment : Fragment() {
     ...
     * field of `sleepTrackerViewModel` whose lambda calls the `submitList` method of `adapter` with
     * that [List] of [SleepNight] objects whenever it is updated to a non-null value (`submitList`
     * will submit the new list to be diffed, and displayed). We then set the LifecycleOwner that
     * should be used for observing changes of [LiveData] in `binding` to `this`.
     *
    ...
    override fun onCreateView(
        ...
        sleepTrackerViewModel.nights.observe(viewLifecycleOwner) { sleepNightList: List<SleepNight> ->
            @Suppress("UNNECESSARY_SAFE_CALL") // If starts out empty, so should be nullable?
            sleepNightList?.let {
