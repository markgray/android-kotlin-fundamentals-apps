...
class MainActivity : AppCompatActivity() {
     ...
     * file `R.layout.activity_main`. We initialize our [LinearLayout] variable
     * `rootView` to the view with ID `R.id.root_view` then call
     * [ViewCompat.setOnApplyWindowInsetsListener] to take over over the policy
     * for applying window insets to `rootView`, with the listener argument a
     * lambda that accepts the [View] passed the lambda in variable `v` and the
     * [WindowInsetsCompat] passed the lambda in variable `windowInsets`. It
     * initializes its [Insets] variable `systemBars` to the [WindowInsetsCompat.getInsets]
