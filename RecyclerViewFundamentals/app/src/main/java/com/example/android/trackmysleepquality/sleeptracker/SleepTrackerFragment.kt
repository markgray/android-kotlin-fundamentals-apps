...
class SleepTrackerFragment : Fragment() {
     ...
     * objects in the `nights` field of `sleepTrackerViewModel` whose lambda sets the dataset
     * field `data` of `adapter` to that [List] of [SleepNight] objects whenever it is updated
     * to a non-null value. We then set the LifecycleOwner that should be used for observing
     * changes of LiveData in `binding` to `this`.
     *
