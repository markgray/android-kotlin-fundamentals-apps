...
class SleepTrackerFragment : Fragment() {
     ...
     *
     * We set the `sleepTrackerViewModel` variable of `binding` to our `sleepTrackerViewModel` to
     * allow the binding to use the view model, then we set the `LifecycleOwner` that should be used
     * for observing changes of [LiveData] in `binding` to `this`.
     *
     * We add an [Observer] to the state variable `showSnackBarEvent` of `sleepTrackerViewModel`
     * for showing a Snackbar message when the CLEAR button is pressed whose lambda shows a
     * [Snackbar] informing the user that we just deleted all of his data, then calls the
