...
class GdgListViewModel : ViewModel() {
     ...
     * (The `Chip` region selection UI is not implemented yet)
     */
     ...
     * by our [onFilterChanged] method when the user changes the filter (when that part of the UI is
     * implemented), and by our [onLocationUpdated] method when the location of the device changes.
     *
    ...
    /**
     * This will be called from the `OnCheckedChangeListener` of each of the `Chip` widgets in the
     * region filter `ChipGroup` when that part of the UI is implemented. We call the `update`
     * method of our [FilterHolder] field [filter] with our parameters and if the value of the its
     * `currentValue` field has changed as a result of the selection change we call our method
     * [onQueryChanged] to reread the list of [GdgChapter] objects based on the new filter.
     *
    ...
    /**
     * Class we use to hold the current region filter, it will eventually be used when the `Chip`
     * region filter UI is added.
     */
    private class FilterHolder {
        ...
        /**
         * Will be called to update the contents of our [currentValue] field when one of the region
         * filter `Chip`'s changes its checked state when that part of the UI is implemented. If
         * [isChecked] is `true` we set our [currentValue] field to our [changedFilter] parameter
         * and return `true`, and if [isChecked] is `false` we set our [currentValue] field to
         * `null` and return `true`. Otherwise we return `false`.
         *
