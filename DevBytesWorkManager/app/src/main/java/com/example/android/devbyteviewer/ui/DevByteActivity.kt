...
class DevByteActivity : AppCompatActivity() {
     ...
     * call [enableEdgeToEdge] to enable edge to edge display. Then we call our super's implementation
     * of `onCreate`, and set our content view to our layout file `R.layout.activity_dev_byte_viewer`.
     * We initialize our [FrameLayout] variable `rootView` to the view with ID `R.id.root_view`,
     * then use the [ViewCompat.setOnApplyWindowInsetsListener] method to set an
     * [OnApplyWindowInsetsListener] on `rootView` to take over over the policy for applying window
     ...
     * the argument, then it updates the layout parameters of `v` to be a [ViewGroup.MarginLayoutParams]
     * with the left margin set to `insets.left`, the right margin set to `insets.right`, the top
     * margin set to `insets.top`, and the bottom margin set to `insets.bottom`. Finally it returns
     * [WindowInsetsCompat.CONSUMED] to the caller (so that the window insets will not keep passing
